...
public class BlockVoxpondsPortal extends Block implements AdventPortalBlock {
	public static final int[][] sides;
	public String name;

	public BlockVoxpondsPortal() {
		super(Material.portal);
		setResistance(999999999f);
		setHardness(-1f);
		setTickRandomly(true);
		setBlockName("voxpondsPortal");
		setBlockTextureName("nevermine:animated/voxpondsPortal");
		setCreativeTab(CreativeTabs.tabBlock);
		GameRegistry.registerBlock(this, "voxpondsPortal");
	}
	...
	public void onEntityCollidedWithBlock(final World par1World, final int par2, final int par3, final int par4, final Entity par5Entity) {
		if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP) {
			final int dim = ConfigurationHelper.voxponds;
			...
			if (thePlayer.timeUntilPortal > 0) {
				...
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterVoxponds(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public static class Size {
		...
		public Size(final World world, final int x, int y, final int z, final int side1) {
			...
			this.world = world;
			this.side1 = side1;
			side1_0 = BlockVoxpondsPortal.sides[side1][0];
			side1_1 = BlockVoxpondsPortal.sides[side1][1];
		...
		protected int getDirectionMaybe() {
			...
			while (directionOrSideMaybe < 21) {
				...
				for (int j = 0; j < rotationOrMaybeNumSides; ++j) {
					final int k = chunkCoords.posX + j * Direction.offsetX[BlockVoxpondsPortal.sides[side1][1]];
					final int l = chunkCoords.posZ + j * Direction.offsetZ[BlockVoxpondsPortal.sides[side1][1]];
					Block block = world.getBlock(k, i, l);
					if (!func_150857_a(block)) {
						break Label_0272;
					}
					if (block == Blockizer.voxpondsPortal) {
						++value;
					}
					if (j == 0) {
						block = world.getBlock(k + Direction.offsetX[BlockVoxpondsPortal.sides[side1][0]], i, l + Direction.offsetZ[BlockVoxpondsPortal.sides[side1][0]]);
						if (block != Blockizer.PlantStem) {
							break Label_0272;
						}
					}
					else if (j == rotationOrMaybeNumSides - 1) {
						block = world.getBlock(k + Direction.offsetX[BlockVoxpondsPortal.sides[side1][1]], i, l + Direction.offsetZ[BlockVoxpondsPortal.sides[side1][1]]);
						if (block != Blockizer.PlantStem) {
							break Label_0272;
						}
					}
			...
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				final int j = chunkCoords.posX + i * Direction.offsetX[BlockVoxpondsPortal.sides[side1][1]];
				final int k = chunkCoords.posY + directionOrSideMaybe;
				final int l = chunkCoords.posZ + i * Direction.offsetZ[BlockVoxpondsPortal.sides[side1][1]];
		...
		protected boolean func_150857_a(final Block block) {
			return block.getMaterial() == Material.air || block == Blockizer.ToxicBlock || block == Blockizer.voxpondsPortal;
		}
		...
		public void func_150859_c() {
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				...
				for (int l = 0; l < directionOrSideMaybe; ++l) {
					...
					world.setBlock(j, i2, k, Blockizer.voxpondsPortal, side1, 2);
				}
			}
		}
	}
