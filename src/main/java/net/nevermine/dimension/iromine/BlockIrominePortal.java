...
public class BlockIrominePortal extends Block implements AdventPortalBlock {
	...
	public BlockIrominePortal() {
		...
		setHardness(-1f);
		setBlockName("irominePortal");
		setBlockTextureName("nevermine:animated/irominePortal");
	...
	public void onEntityCollidedWithBlock(final World par1World, final int par2, final int par3, final int par4, final Entity par5Entity) {
		if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP) {
			final int dim = ConfigurationHelper.iromine;
			...
			if (thePlayer.timeUntilPortal > 0) {
				...
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, dim, new TeleporterIromine(thePlayer.mcServer.worldServerForDimension(dim)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterIromine(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public static class Size {
		...
		public Size(final World world, final int x, int y, final int z, final int side1) {
			...
			this.world = world;
			this.side1 = side1;
			side1_0 = BlockIrominePortal.sides[side1][0];
			side1_1 = BlockIrominePortal.sides[side1][1];
		...
		protected int getValueBasedOnBlock(final int x, final int y, final int z, final int side) {
			...
			for (i1 = 0; i1 < 22; ++i1) {
				...
				if (block2 != Blocks.gold_block) {
					break;
				}
			}
			final Block block = world.getBlock(x + j1 * i1, y, z + k1 * i1);
			return (block == Blocks.gold_block) ? i1 : 0;
		}
		...
		protected int getDirectionMaybe() {
			...
			while (directionOrSideMaybe < 21) {
				...
				for (int j = 0; j < rotationOrMaybeNumSides; ++j) {
					final int k = chunkCoords.posX + j * Direction.offsetX[BlockIrominePortal.sides[side1][1]];
					final int l = chunkCoords.posZ + j * Direction.offsetZ[BlockIrominePortal.sides[side1][1]];
					Block block = world.getBlock(k, i, l);
					if (!func_150857_a(block)) {
						break Label_0272;
					}
					if (block == Blockizer.irominePortal) {
						++value;
					}
					if (j == 0) {
						block = world.getBlock(k + Direction.offsetX[BlockIrominePortal.sides[side1][0]], i, l + Direction.offsetZ[BlockIrominePortal.sides[side1][0]]);
						if (block != Blocks.gold_block) {
							break Label_0272;
						}
					}
					else if (j == rotationOrMaybeNumSides - 1) {
						block = world.getBlock(k + Direction.offsetX[BlockIrominePortal.sides[side1][1]], i, l + Direction.offsetZ[BlockIrominePortal.sides[side1][1]]);
						if (block != Blocks.gold_block) {
							break Label_0272;
						}
					}
			...
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				final int j = chunkCoords.posX + i * Direction.offsetX[BlockIrominePortal.sides[side1][1]];
				final int k = chunkCoords.posY + directionOrSideMaybe;
				final int l = chunkCoords.posZ + i * Direction.offsetZ[BlockIrominePortal.sides[side1][1]];
				if (world.getBlock(j, k, l) != Blocks.gold_block) {
					directionOrSideMaybe = 0;
					break;
				}
			}
		...
		protected boolean func_150857_a(final Block block) {
			return block.getMaterial() == Material.air || block == Blockizer.ToxicBlock || block == Blockizer.irominePortal;
		}
		...
		public void func_150859_c() {
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				...
				for (int l = 0; l < directionOrSideMaybe; ++l) {
					...
					world.setBlock(j, i2, k, Blockizer.irominePortal, side1, 2);
				}
			}
		}
	}
