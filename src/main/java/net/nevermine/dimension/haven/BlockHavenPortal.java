...
import net.nevermine.fx.portal.EntityHavenPortalFX;
import net.nevermine.izer.Blockizer;

import java.util.Random;

public class BlockHavenPortal extends Block implements AdventPortalBlock {
	public static final int[][] sides;
	public String name;

	public BlockHavenPortal() {
		super(Material.portal);
		setResistance(999999999f);
		setHardness(-1f);
		setTickRandomly(true);
		setBlockName("havenPortal");
		setBlockTextureName("nevermine:animated/havenPortal");
		setCreativeTab(CreativeTabs.tabBlock);
		GameRegistry.registerBlock(this, "havenPortal");
	}
	...
	public void onEntityCollidedWithBlock(final World par1World, final int par2, final int par3, final int par4, final Entity par5Entity) {
		if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP) {
			final int dim = ConfigurationHelper.haven;
			...
			if (thePlayer.timeUntilPortal > 0) {
				...
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, dim, new TeleporterHaven(thePlayer.mcServer.worldServerForDimension(dim)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterHaven(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public void randomDisplayTick(final World world, final int xPos, final int yPos, final int zPos, final Random rand) {
		for (int l = 0; l < 4; ++l) {
			...
			final EntityHavenPortalFX var20 = new EntityHavenPortalFX(world, d0, d2, d3, d4, d5, d6);
	...
	public static class Size {
		...
		public Size(final World world, final int x, int y, final int z, final int side1) {
			...
			this.world = world;
			this.side1 = side1;
			side1_0 = BlockHavenPortal.sides[side1][0];
			side1_1 = BlockHavenPortal.sides[side1][1];
		...
		protected int getDirectionMaybe() {
			...
			while (directionOrSideMaybe < 21) {
				...
				for (int j = 0; j < rotationOrMaybeNumSides; ++j) {
					final int k = chunkCoords.posX + j * Direction.offsetX[BlockHavenPortal.sides[side1][1]];
					final int l = chunkCoords.posZ + j * Direction.offsetZ[BlockHavenPortal.sides[side1][1]];
					...
					if (j == 0) {
						block = world.getBlock(k + Direction.offsetX[BlockHavenPortal.sides[side1][0]], i, l + Direction.offsetZ[BlockHavenPortal.sides[side1][0]]);
						if (block != Blockizer.TwinkleStone) {
							break Label_0272;
						}
					}
					else if (j == rotationOrMaybeNumSides - 1) {
						block = world.getBlock(k + Direction.offsetX[BlockHavenPortal.sides[side1][1]], i, l + Direction.offsetZ[BlockHavenPortal.sides[side1][1]]);
						if (block != Blockizer.TwinkleStone) {
							break Label_0272;
						}
					}
			...
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				final int j = chunkCoords.posX + i * Direction.offsetX[BlockHavenPortal.sides[side1][1]];
				final int k = chunkCoords.posY + directionOrSideMaybe;
				final int l = chunkCoords.posZ + i * Direction.offsetZ[BlockHavenPortal.sides[side1][1]];
		...
		protected boolean func_150857_a(final Block block) {
			return block.getMaterial() == Material.air || block == Blockizer.ToxicBlock || block == Blockizer.havenPortal;
		}
		...
		public void func_150859_c() {
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				...
				for (int l = 0; l < directionOrSideMaybe; ++l) {
					...
					world.setBlock(j, i2, k, Blockizer.havenPortal, side1, 2);
				}
			}
		}
	}
