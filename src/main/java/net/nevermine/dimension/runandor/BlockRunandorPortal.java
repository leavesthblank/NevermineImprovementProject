...
import net.nevermine.fx.portal.EntityRunicPortalFX;
import net.nevermine.izer.Blockizer;
import net.nevermine.izer.Itemizer;

import java.util.Random;

public class BlockRunandorPortal extends Block implements AdventPortalBlock {
	public static final int[][] sides;
	public String name;

	public BlockRunandorPortal() {
		super(Material.portal);
		setResistance(999999999f);
		setHardness(-1f);
		setTickRandomly(true);
		setBlockName("runandorPortal");
		setBlockTextureName("nevermine:animated/runandorPortal");
		setCreativeTab(CreativeTabs.tabBlock);
		GameRegistry.registerBlock(this, "runandorPortal");
	}
	...
	public void onEntityCollidedWithBlock(final World par1World, final int par2, final int par3, final int par4, final Entity par5Entity) {
		if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP) {
			final int dim = ConfigurationHelper.runandor;
			...
			if (thePlayer.timeUntilPortal > 0) {
				...
				if (thePlayer.capabilities.isCreativeMode || thePlayer.inventory.hasItem(Itemizer.DimensionTicket)) {
					...
					thePlayer.timeUntilPortal = 10;
					thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, dim, new TeleporterRunandor(thePlayer.mcServer.worldServerForDimension(dim)));
				}
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, ConfigurationHelper.mysterium, new TeleporterRunandor(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public void randomDisplayTick(final World world, final int xPos, final int yPos, final int zPos, final Random rand) {
		for (int l = 0; l < 4; ++l) {
			...
			final EntityRunicPortalFX var20 = new EntityRunicPortalFX(world, d0, d2, d3, d4, d5, d6);
	...
	public static class Size {
		...
		public Size(final World world, final int x, int y, final int z, final int side1) {
			...
			this.world = world;
			this.side1 = side1;
			side1_0 = BlockRunandorPortal.sides[side1][0];
			side1_1 = BlockRunandorPortal.sides[side1][1];
		...
		protected int getValueBasedOnBlock(final int x, final int y, final int z, final int side) {
			...
			for (i1 = 0; i1 < 22; ++i1) {
				...
				if (block2 != Blockizer.RunicBrick) {
					break;
				}
			}
			final Block block = world.getBlock(x + j1 * i1, y, z + k1 * i1);
			return (block == Blockizer.RunicBrick) ? i1 : 0;
		}
		...
		protected int getDirectionMaybe() {
			...
			while (directionOrSideMaybe < 21) {
				...
				for (int j = 0; j < rotationOrMaybeNumSides; ++j) {
					final int k = chunkCoords.posX + j * Direction.offsetX[BlockRunandorPortal.sides[side1][1]];
					final int l = chunkCoords.posZ + j * Direction.offsetZ[BlockRunandorPortal.sides[side1][1]];
					Block block = world.getBlock(k, i, l);
					if (!func_150857_a(block)) {
						break Label_0272;
					}
					if (block == Blockizer.overgrowthPortal) {
						++value;
					}
					if (j == 0) {
						block = world.getBlock(k + Direction.offsetX[BlockRunandorPortal.sides[side1][0]], i, l + Direction.offsetZ[BlockRunandorPortal.sides[side1][0]]);
						if (block != Blockizer.RunicBrick) {
							break Label_0272;
						}
					}
					else if (j == rotationOrMaybeNumSides - 1) {
						block = world.getBlock(k + Direction.offsetX[BlockRunandorPortal.sides[side1][1]], i, l + Direction.offsetZ[BlockRunandorPortal.sides[side1][1]]);
						if (block != Blockizer.RunicBrick) {
							break Label_0272;
						}
					}
			...
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				final int j = chunkCoords.posX + i * Direction.offsetX[BlockRunandorPortal.sides[side1][1]];
				final int k = chunkCoords.posY + directionOrSideMaybe;
				final int l = chunkCoords.posZ + i * Direction.offsetZ[BlockRunandorPortal.sides[side1][1]];
				if (world.getBlock(j, k, l) != Blockizer.RunicBrick) {
					directionOrSideMaybe = 0;
					break;
				}
			}
		...
		protected boolean func_150857_a(final Block block) {
			return block.getMaterial() == Material.air || block == Blockizer.ToxicBlock || block == Blockizer.overgrowthPortal;
		}
		...
		public void func_150859_c() {
			for (int i = 0; i < rotationOrMaybeNumSides; ++i) {
				...
				for (int l = 0; l < directionOrSideMaybe; ++l) {
					...
					world.setBlock(j, i2, k, Blockizer.overgrowthPortal, side1, 2);
				}
			}
		}
	}
