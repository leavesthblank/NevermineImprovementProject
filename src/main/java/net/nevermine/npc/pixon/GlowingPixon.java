...
public class GlowingPixon extends EntityAnimal {
	public GlowingPixon(final World par1World) {
		super(par1World);
		setSize(0.9f, 1.3f);
	...
	public boolean interact(final EntityPlayer par1EntityPlayer) {
		PixonCollection.collect(this, par1EntityPlayer, 200, 40, Itemizer.iStoneGlowing);
		return super.interact(par1EntityPlayer);
	}

	public EntityAgeable createChild(final EntityAgeable var1) {
		return new GlowingPixon(worldObj);
	}
	...
	public void onUpdate() {
		...
		if (worldObj.isRemote) {
			for (int var3 = 0; var3 < 8; ++var3) {
				final RedTrail var4 = new RedTrail(worldObj, posX, posY + 1.0, posZ, 0.0, 0.0, 0.0, 5);
