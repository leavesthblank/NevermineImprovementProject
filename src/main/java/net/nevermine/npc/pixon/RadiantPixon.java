...
public class RadiantPixon extends EntityAnimal {
	public RadiantPixon(final World par1World) {
		super(par1World);
		setSize(0.9f, 1.3f);
	...
	public boolean interact(final EntityPlayer par1EntityPlayer) {
		PixonCollection.collect(this, par1EntityPlayer, 700, 72, Itemizer.iStoneRadiant);
		return super.interact(par1EntityPlayer);
	}

	public EntityAgeable createChild(final EntityAgeable var1) {
		return new RadiantPixon(worldObj);
	}
	...
	public void onUpdate() {
		...
		if (worldObj.isRemote) {
			for (int var3 = 0; var3 < 8; ++var3) {
				final BlackTrail var4 = new BlackTrail(worldObj, posX, posY + 1.0, posZ, 0.0, 0.0, 0.0, 5);
