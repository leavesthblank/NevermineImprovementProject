...
public class EntityLottomanCeleve extends EntityNevermineVillager {
	private static ArrayList<RestockedRecipe> trades = new ArrayList<RestockedRecipe>();

	public EntityLottomanCeleve(final World var1) {
		super(var1);
	}
	...
	public void interaction(final EntityPlayer p) {
		p.addChatMessage(StringUtil.getLocale("message.dialogue.lottoManCeleve." + rand.nextInt(5)));
	}
	...
	public void addRecipies(final MerchantRecipeList list) {
		list.add(new RestockedRecipe(new ItemStack(Itemizer.CoinsCeleve, 10, 1), new ItemStack(Totemizer.TotemGoofyGreatblade)));
