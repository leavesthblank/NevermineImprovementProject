...
public class EntityStinger extends EntityAIFlying {
	private ChunkCoordinates currentFlightTarget;
	private int flyTimer;

	public EntityStinger(final World var1) {
		super(var1);
	...
	protected String getLivingSound() {
		return "nevermine:StingerLiving";
	}

	protected String getHurtSound() {
		return "nevermine:StingerHit";
	}
	...
	public boolean attackEntityAsMob(final Entity par1) {
		if (super.attackEntityAsMob(par1)) {
			if (par1 instanceof EntityLivingBase) {
				((EntityLivingBase)par1).addPotionEffect(new PotionEffect(Potion.poison.id, 230, 3));
			}
			return true;
		}
		return false;
