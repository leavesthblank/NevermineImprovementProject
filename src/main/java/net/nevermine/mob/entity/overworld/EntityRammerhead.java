...
public class EntityRammerhead extends EntityMob{

	public EntityRammerhead(final World par1World) {
		super(par1World);
		setSize(3.2f, 2.3f);
	}

	protected String getLivingSound() {
		return "nevermine:RammerheadLiving";
	}

	protected String getDeathSound() {
		return "nevermine:RammerheadDeath";
	}

	protected String getHurtSound() {
		return "nevermine:RammerheadHit";
	}
	...
	public boolean attackEntityAsMob(final Entity par1Entity) {
		...
		if (entityToAttack != null) {
			if (entityToAttack instanceof EntityPlayer && ((EntityPlayer)entityToAttack).getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue() != 1.0) {
				entityToAttack.addVelocity(motionX * 20.5, motionY * 0.5, motionZ * 22.0);
			}
			return true;
		}
		return false;
