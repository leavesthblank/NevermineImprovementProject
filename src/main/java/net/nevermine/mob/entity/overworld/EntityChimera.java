...
public class EntityChimera extends EntityMob{
	...
	protected String getLivingSound() {
		return "nevermine:ChimeraLiving";
	}

	protected String getDeathSound() {
		return "nevermine:ChimeraDeath";
	}

	protected String getHurtSound() {
		return "nevermine:ChimeraHit";
	}
	...
	public boolean attackEntityAsMob(final Entity par1) {
		if (super.attackEntityAsMob(par1)) {
			if (par1 instanceof EntityLivingBase) {
				((EntityLivingBase)par1).addPotionEffect(new PotionEffect(Potion.weakness.id, 240, 2));
			}
			return true;
		}
		return false;
