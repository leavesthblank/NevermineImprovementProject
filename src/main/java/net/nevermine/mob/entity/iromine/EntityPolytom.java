...
public class EntityPolytom extends EntityFlying implements IMob {
	...
	public EntityPolytom(final World par1World) {
		super(par1World);
		explosionStrength = 3;
		setSize(1.5f, 1.5f);
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null && targetedEntity.getDistanceSqToEntity(this) < d5 * d5) {
			...
			if (canEntityBeSeen(targetedEntity)) {
				++attackCounter;
				if (attackCounter == 50) {
					final EntityPolytomShot entitylargefireball = new EntityPolytomShot(worldObj, this);
	...
	protected String getLivingSound() {
		return "nevermine:PolytomLiving";
	}

	protected String getHurtSound() {
		return "nevermine:PolytomHit";
	}

	protected String getDeathSound() {
		return "nevermine:PolytomDeath";
	}
