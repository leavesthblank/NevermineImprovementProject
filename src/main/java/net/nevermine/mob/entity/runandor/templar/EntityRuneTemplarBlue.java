...
public class EntityRuneTemplarBlue extends EntityCreature {
	public EntityRuneTemplarBlue(final World par1World) {
		super(par1World);
		setSize(1.79f, 2.2f);
	}
	...
	private void transform() {
		if (!worldObj.isRemote) {
			final EntityRuneTemplarBlueDisabled var2 = new EntityRuneTemplarBlueDisabled(worldObj);
	...
	public void onLivingUpdate() {
		...
		final int multiplier = worldObj.getEntitiesWithinAABB(EntityRunicLifeformBlue.class, boundingBox.expand(10.0, 10.0, 10.0)).size();
		...
		if (getHealth() < 2.5f) {
			if (!worldObj.isRemote) {
				dropItem(Itemizer.MegaRuneFragmentB, 1);
			}
			transform();
		}
		else {
		...
		if (rand.nextInt(125) == 25 && !worldObj.isRemote) {
			final EntityRunicLifeformBlue var2 = new EntityRunicLifeformBlue(worldObj);
