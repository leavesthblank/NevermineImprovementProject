...
public class EntityRuneTemplarRed extends EntityCreature {
	...
	public void onLivingUpdate() {
		...
		final int multiplier = worldObj.getEntitiesWithinAABB(EntityRunicLifeformRed.class, boundingBox.expand(10.0, 10.0, 10.0)).size();
		...
		if (rand.nextInt(100) == 25 && !worldObj.isRemote) {
			final EntityRunicLifeformRed var2 = new EntityRunicLifeformRed(worldObj);
	...
	private void transform() {
		if (!worldObj.isRemote) {
			final EntityRuneTemplarRedDisabled var2 = new EntityRuneTemplarRedDisabled(worldObj);
