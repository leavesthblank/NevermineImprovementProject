...
public class EntityNightwing extends EntityAIFlying {
	private ChunkCoordinates currentFlightTarget;
	private int flyTimer;

	public EntityNightwing(final World var1) {
		super(var1);
	...
	protected void dropFewItems(final boolean par1, final int par2) {
		if (rand.nextInt(100) <= 5) {
			dropItem(Weaponizer.GasBlaster, 1);
		}
		if (rand.nextInt(5) == 2) {
			dropItem(Itemizer.ApocoStone, 1);
		}
	...
	public boolean getCanSpawnHere() {
		return worldObj.difficultySetting != EnumDifficulty.PEACEFUL && posY > 16.0 && worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox).isEmpty() && !worldObj.isAnyLiquid(boundingBox);
	}
	...
	protected String getLivingSound() {
		return "nevermine:NightwingLiving";
	}

	protected String getHurtSound() {
		return "nevermine:NightwingHit";
	}

	protected String getDeathSound() {
		return "nevermine:NightwingDeath";
	}
