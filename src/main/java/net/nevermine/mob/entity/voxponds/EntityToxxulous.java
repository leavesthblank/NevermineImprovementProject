...
public class EntityToxxulous extends EntityMob {
	public EntityToxxulous(final World par1World) {
		super(par1World);
		setSize(1.2f, 1.6f);
	}

	protected String getLivingSound() {
		return "nevermine:ToxxulousLiving";
	}

	protected String getDeathSound() {
		return "nevermine:ToxxulousDeath";
	}

	protected String getHurtSound() {
		return "nevermine:ToxxulousHit";
	}
	...
	public void onLivingUpdate() {
		...
		for (final EntityPlayer e : (List<EntityPlayer>)worldObj.getEntitiesWithinAABB(EntityPlayer.class, boundingBox.expand(7.0, 7.0, 7.0))) {
			...
			e.addPotionEffect(new PotionEffect(Potion.poison.id, 30, 2));
		}
	}
