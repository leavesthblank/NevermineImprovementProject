...
public class EntitySticky extends EntityMob {
	public EntitySticky(final World par1World) {
		super(par1World);
		setSize(0.6f, 2.0f);
	}
	...
	protected String getLivingSound() {
		...
		if (pick == 5) {
			return "nevermine:CeleveClownLiving6";
		}
	...
	public void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(95.0);
	}

	public boolean attackEntityAsMob(final Entity par1) {
		if (super.attackEntityAsMob(par1)) {
			if (par1 instanceof EntityLivingBase) {
				((EntityLivingBase)par1).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 60, 30));
			}
			return true;
		}
		return false;
