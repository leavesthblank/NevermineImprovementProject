...
public class EntityAirhead extends EntityFlying implements IMob {
	...
	public EntityAirhead(final World par1World) {
		super(par1World);
		explosionStrength = 3;
		setSize(1.1f, 1.6f);
	...
	public boolean getCanSpawnHere() {
		return worldObj.difficultySetting != EnumDifficulty.PEACEFUL && worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox).isEmpty() && !worldObj.isAnyLiquid(boundingBox) && rand.nextInt(10) == 3;
	}
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null && targetedEntity.getDistanceSqToEntity(this) < d5 * d5) {
			...
			if (canEntityBeSeen(targetedEntity)) {
				++attackCounter;
				if (attackCounter == 20) {
					final EntityAirheadShot entitylargefireball = new EntityAirheadShot(worldObj, this);
	...
	protected String getLivingSound() {
		return "nevermine:AirheadLiving";
	}

	protected String getHurtSound() {
		return "nevermine:AirheadHit";
	}

	protected String getDeathSound() {
		return "nevermine:AirheadDeath";
	}
