...
public class EntityVoliant extends EntityFlying implements IMob {
	...
	public EntityVoliant(final World par1World) {
		super(par1World);
		explosionStrength = 3;
		setSize(4.0f, 4.0f);
	...
	protected void dropFewItems(final boolean par1, final int par2) {
		if (rand.nextInt(1) == 0) {
			dropItem(Itemizer.CoinsHaven, 5);
		}
		dropItem(Itemizer.VoliantHeart, 1);
		if (rand.nextInt(2) == 1) {
			dropItem(Weaponizer.RPG, 1);
		}
		dropItem(Itemizer.RealmstoneHaven, 3);
	}

	public boolean getCanSpawnHere() {
		return worldObj.difficultySetting != EnumDifficulty.PEACEFUL && worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox).isEmpty() && !worldObj.isAnyLiquid(boundingBox) && rand.nextInt(15) == 6;
	}
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null && targetedEntity.getDistanceSqToEntity(this) < d5 * d5) {
			...
			if (canEntityBeSeen(targetedEntity)) {
				++attackCounter;
				if (attackCounter == 5) {
					final EntityVoliantShot entitylargefireball = new EntityVoliantShot(worldObj, this);
	...
	protected String getLivingSound() {
		return "nevermine:VoliantLiving";
	}

	protected String getHurtSound() {
		return "nevermine:VoliantHit";
	}

	protected String getDeathSound() {
		return "nevermine:VoliantDeath";
	}
