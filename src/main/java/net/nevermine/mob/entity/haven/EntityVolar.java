...
public class EntityVolar extends EntityFlying implements IMob {
	...
	public EntityVolar(final World par1World) {
		super(par1World);
		explosionStrength = 3;
		setSize(1.0f, 1.0f);
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null && targetedEntity.getDistanceSqToEntity(this) < d5 * d5) {
			...
			if (canEntityBeSeen(targetedEntity)) {
				++attackCounter;
				if (attackCounter == 20) {
					final EntityVolarShot entitylargefireball = new EntityVolarShot(worldObj, this);
	...
	protected String getLivingSound() {
		return "nevermine:VolarLiving";
	}

	protected String getHurtSound() {
		return "nevermine:VolarHit";
	}

	protected String getDeathSound() {
		return "nevermine:VolarDeath";
	}
