...
public class EntityValkyrie extends EntityFlying implements IMob {
	...
	public EntityValkyrie(final World par1World) {
		super(par1World);
		explosionStrength = 3;
		setSize(1.0f, 1.0f);
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null && targetedEntity.getDistanceSqToEntity(this) < d5 * d5) {
			...
			if (canEntityBeSeen(targetedEntity)) {
				++attackCounter;
				if (attackCounter == 30) {
					final EntityValkyrieShot entitylargefireball = new EntityValkyrieShot(worldObj, this);
	...
	protected String getLivingSound() {
		return "nevermine:ValkyrieLiving";
	}

	protected String getHurtSound() {
		return "nevermine:ValkyrieHit";
	}

	protected String getDeathSound() {
		return "nevermine:ValkyrieDeath";
	}
