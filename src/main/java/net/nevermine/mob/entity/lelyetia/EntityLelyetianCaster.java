...
public class EntityLelyetianCaster extends EntityMob implements IRangedAttackMob {
	private EntityAIArrowAttack aiArrowAttack;

	public EntityLelyetianCaster(final World par1World) {
		super(par1World);
	...
	protected String getDeathSound() {
		return "nevermine:LelyetianDeath";
	}
	...
	protected void dropFewItems(final boolean par1, final int par2) {
		if (rand.nextInt(6) == 1) {
			dropItem(Plantizer.ChiliSeeds, 1);
		}
	...
	public void attackEntityWithRangedAttack(final EntityLivingBase var1, final float f) {
		final EntityLelyetianShot var2 = new EntityLelyetianShot(worldObj, this, 4.0f, 1);
		...
		playSound("nevermine:LelyetianCaster", 1.0f, 1.0f / (getRNG().nextFloat() * 0.4f + 0.8f));
