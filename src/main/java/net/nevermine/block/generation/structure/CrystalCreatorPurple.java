...
public class CrystalCreatorPurple extends Block {
	private Random rand;

	public CrystalCreatorPurple(final Material p_i45394_1_) {
		super(p_i45394_1_);
		rand = new Random();
	...
	public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p, final int var6, final float var7, final float var8, final float var9) {
		if (!w.isRemote && p.getHeldItem() != null && p.getHeldItem().getItem() == Itemizer.GemstonesPurple) {
			p.inventory.consumeInventoryItem(Itemizer.GemstonesPurple);
			if (!w.isRemote) {
				p.dropItem(Itemizer.CrystalsPurple, 1);
